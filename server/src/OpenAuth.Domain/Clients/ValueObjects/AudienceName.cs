using System.Diagnostics.CodeAnalysis;
using OpenAuth.Domain.Shared.ValueObjects;

namespace OpenAuth.Domain.Clients.ValueObjects;

public record AudienceName : Name
{
    public AudienceName(string value) : base(value, 1, 100) { }
    
    public static AudienceName From(string value)
    {
        if (!TryFrom(value, out var name))
            throw new ArgumentException($"Invalid audience name: {value}", nameof(value));

        return name;
    }

    public static bool TryFrom(string? value, [NotNullWhen(true)] out AudienceName? name)
    {
        name = null;
        
        if (string.IsNullOrWhiteSpace(value))
            return false;
        
        try
        {
            name = new AudienceName(value);
            return true;
        }
        catch(ArgumentException _)
        {
            return false;
        }
    }
    
    public override string ToString() => Value;
}