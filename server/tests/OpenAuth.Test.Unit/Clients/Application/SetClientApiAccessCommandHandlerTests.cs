...
public class SetClientApiAccessCommandHandlerTests
{
    private sealed class TestContext
    {
        ...
        public SetClientApiAccessCommandHandler Sut { get; }
        ...
        public TestContext()
        {
            Sut = new SetClientApiAccessCommandHandler(Apis, Clients, Time);
        ...
        {
            var command = new SetClientApiAccessCommand(
                Client.Id,
                Api.Id,
                ScopeCollection.Parse(scopes ?? DefaultValues.Scopes));
    ...
    [Fact]
    public async Task HandleAsync_WhenNoExistingAccess_CreatesAccess()
    {
    ...
    [Fact]
    public async Task HandleAsync_WhenAccessAlreadyExists_UpdatesScopes()
    {
        var context = new TestContext();
        context.WithApi(builder => builder.WithScopes("read write"));

        await context.HandleAsync("read");
        await context.HandleAsync("write");
    ...
    [Fact]
    public async Task HandleAsync_WhenScopesEmpty_RemovesAccess()
    {
        var context = new TestContext();
        context.WithApi(builder => builder.WithScopes("read write"));

        await context.HandleAsync("read");
        await context.HandleAsync(""); // empty = revoke

        Assert.Empty(context.Client.Apis);
    }

    [Fact]
    public async Task HandleAsync_WhenScopesEmptyAndNoAccess_NoOp()
    {
        var context = new TestContext();
    ...
    public async Task HandleAsync_WhenClientNotFound_Throws()
    {
        ...
        context.Clients.GetByIdAsync(context.Client.Id, Arg.Any<CancellationToken>())
            .Returns((Client?)null);
    ...
    public async Task HandleAsync_WhenApiNotFound_Throws()
    {
        ...
        context.Apis.GetByIdAsync(context.Api.Id, Arg.Any<CancellationToken>())
        ...
        await Assert.ThrowsAsync<ApiResourceNotFoundException>(() =>
            context.Sut.HandleAsync(command, CancellationToken.None));
    }
}